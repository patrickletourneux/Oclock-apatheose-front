...
function RankingTile({ data, hasHome }) {
  ...
  const displayWithHome = () => (
    <Tile>
      ...
      <Box display="flex" justifyContent="space-evenly" flexWrap="nowrap" gap="1rem" marginTop="2rem">
        <FirstRankingSecondaryTile data={data.firstUser} isCurrentUserFirst={isCurrentUserFirst} />
  ...
  const displayNoHome = () => (
    <Tile>
      ...
      <Typography variant="h3" color="error" textAlign="center" marginTop="2rem">
        Vous n'avez pas encore de maison associée, vous n'avez pas de classement pour le moment
      </Typography>
    </Tile>
  );
